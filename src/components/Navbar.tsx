...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      // Creates a trigger zone from 40% to 45% down the viewport.
      // When a section's top edge enters this band the nav item activates.
  ...
  return (
    <nav className="glass-nav fixed inset-x-0 top-0 z-50 backdrop-blur-md bg-black/30 border-b border-white/10">
              ...
              {navItems.map((item) => {
                ...
                return (
                  <a
                    key={item.name}
                    href={item.href}
                    className={`text-sm font-medium px-3 py-2 transition-all duration-200 border-b-2 ${
                      isActive
                        ? 'text-emerald-300 border-emerald-400'
                        : 'border-transparent text-slate-300 hover:text-emerald-300 hover:bg-emerald-500/10 rounded-md'
                    }`}
                  >
                    {item.name}
                  </a>
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div id="mobile-menu" className="md:hidden glass-panel border-t border-emerald-500/30">
